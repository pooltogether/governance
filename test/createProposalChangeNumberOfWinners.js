...
async function run() {
    ...
    dim(`Proposing from address ${MultiSig}...`)
    ...
    await pool.delegate(MultiSig)
    green("Balance of timelock before proposal : ", treasuryBalanceBeforeClaim)

    
    const DAI_PRIZE_STRATEGY = '0x178969A87a78597d303C47198c66F68E8be67Dc2'

    // create a proposal to call claim() on Treasury Vestor
    dim(`creating proposal with target ${treasuryVestingAddress}`)   
    const callData = await ethers.utils.defaultAbiCoder.encode(['uint256'], ['10'])
    const createProposalTx =   await alphaGovernanceContract.propose([DAI_PRIZE_STRATEGY], ['0'], ["setNumberOfWinners(uint256)"], [callData], "set number of winners")
    ...
    dim(`fast forwarding ${votingPeriod.toNumber()} blocks... may take a few minutes`)
